...
uint16_t angleToPWM(float degrees) {
  const uint16_t minPulse = 120;  // pulse count for 0°
  const uint16_t maxPulse = 550;  // pulse count for 180°
  degrees = constrain(degrees, 0, 180);
  return map(degrees, 0, 180, minPulse, maxPulse);
}

void sweepChannel(uint8_t channel, uint8_t reps = 3, uint8_t angleA = 30, uint8_t angleB = 150, uint16_t delayMs = 600) {
  for (int i = 0; i < reps; i++) {
    pwm.setPWM(channel, 0, angleToPWM(angleA));
    delay(delayMs);
    pwm.setPWM(channel, 0, angleToPWM(angleB));
    delay(delayMs);
  }
...
void runRoutine_sweep() {
  ...
  logEvent("🔄 CH07 Swing Test: 3x @ 120°");
  sweepChannel(7, 3, 30, 150);
}
